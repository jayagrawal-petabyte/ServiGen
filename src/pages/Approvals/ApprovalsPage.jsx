...
export default function ApprovalsPage() {
  ...
  const [toast, setToast] = useState(null); // { message, lastAction: { id, prevStatus } }

  // Filtered approvals based on tab and search
  ...
  const showToast = (message, lastAction = null) => {
  ...
  const handleUndo = () => {
    ...
    setApprovals((prev) =>
      prev.map((app) => (app.id === id ? { ...app, status: prevStatus } : app))
    );
    setToast(null);
  };
  ...
  return (
    <div className="approvals-page">
      ...
      {/* Sub-bar with Filter Tabs and Search */}
      ...
      {isLoading ? (
        <div className="approvals-list">
          {[1, 2, 3].map((i) => (
            <div key={i} className="approval-card" style={{ padding: '24px' }}>
              <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
                <div className="approval-skeleton" style={{ width: '44px', height: '44px', borderRadius: '50%' }} />
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div className="approval-skeleton" style={{ width: '220px', height: '16px' }} />
                  <div className="approval-skeleton" style={{ width: '380px', height: '14px' }} />
                </div>
              </div>
              <div className="approval-skeleton" style={{ width: '90%', height: '40px', marginLeft: '60px', marginBottom: '16px' }} />
        ...
        <div className="approvals-list">
          {filteredApprovals.map((approval) => (
            <ApprovalCard
              ...
              onOpenDetails={(item) => setSelectedApproval(item)}
            />
          ))}
        </div>
      )}
